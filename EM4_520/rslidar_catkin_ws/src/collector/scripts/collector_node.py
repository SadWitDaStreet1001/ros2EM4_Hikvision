...
class SynchronizedCollector:
    def __init__(self):
        ...
        self.image_topic = rospy.get_param("~image_topic", "/collector/camera/image_raw")
        ...
        rospy.loginfo("collector_node subscribing image=%s cloud=%s", self.image_topic, self.cloud_topic)
        rospy.loginfo("collector_node saving data to %s", self.data_dir)
    ...
    def _image_callback(self, msg):
        ...
        cv2.imshow("collector synchronized image", image)
        cv2.imshow("collector lidar top view", cloud_view)
    ...
    def _write_pcd_ascii(self, path, cloud):
        ...
        if intensity_field and self.require_nonzero_intensity and nonzero_intensity_points == 0:
            rospy.logerr(
                ...
                "Check /rslidar_points intensity values before calibration collection."
            )
            return None

...
from PythonQt.paraview import vvCalibrationDialog, vvCropReturnsDialog, vvSelectFramesDialog, vvConnectInformationDialog, \
                              vvNetworkConfigSensorDialog, vvNetworkConfigPcapDialog, vvLaserSelectionDialog, vvPointsSelectionDialog, vvColumnSelectionDialog
...
class AppLogic(object):
    ...
    def setupTimers(self):
        ...
        self.hardwareTimer = QtCore.QTimer()
        self.hardwareTimer.setSingleShot(True)
        self.hardwareTimer.connect('timeout()', outputHvTimer)

        self.hardwareTimerForSensor = QtCore.QTimer()
        self.hardwareTimerForSensor.setSingleShot(True)
        self.hardwareTimerForSensor.connect('timeout()', changeSelectionTimer)

        self.compRateTimer = QtCore.QTimer()
        self.compRateTimer.setSingleShot(True)
        self.compRateTimer.connect('timeout()', outputCompRateTimer)

        self.speedTimer = QtCore.QTimer()
        self.speedTimer.setSingleShot(True)
        self.speedTimer.connect('timeout()', outputSpeedTimer)

        self.pktLossTimer = QtCore.QTimer()
        self.pktLossTimer.setSingleShot(True)
        self.pktLossTimer.connect('timeout()', outputPktLossTimer)
    ...
    def createStatusBarWidgets(self):
        ...
        self.hardwareLabel = QtGui.QLabel()
        self.compRateLabel = QtGui.QLabel()
        self.speedLabel = QtGui.QLabel()
        self.pktLossLabel = QtGui.QLabel()
...
class IconPaths(object):
    ...
    pos_x_label = ':/RSlidar16Plugin/modify-pos-x.png'
    pos_y_label = ':/RSlidar16Plugin/modify-pos-y.png'
    pos_z_label = ':/RSlidar16Plugin/modify-pos-z.png'
    pos_pitch_label = ':/RSlidar16Plugin/modify-pos-pitch.png'
    pos_roll_label = ':/RSlidar16Plugin/modify-pos-roll.png'
    pos_yaw_label = ':/RSlidar16Plugin/modify-pos-yaw.png'
...
def openData(filename):
    ...
    # custom 0620
    colorByIntensityAndRings(reader)
...
def colorByIntensityAndRings(sourceProxy):

    if not hasArrayName(sourceProxy, 'intensity'):
        return False

    if not hasArrayName(sourceProxy, 'ring'):
        return False

    setDefaultLookupTables(sourceProxy)
    rep = smp.GetDisplayProperties(sourceProxy)

    if hasArrayName(sourceProxy, 'noise'):
        rep.ColorArrayName = 'noise'
        rep.LookupTable = smp.GetLookupTableForArray('noise', 1)

    if hasArrayName(sourceProxy, 'interference'):
        rep.ColorArrayName = 'interference'
        rep.LookupTable = smp.GetLookupTableForArray('interference', 1)

    rep.ColorArrayName = 'ring'
    rep.LookupTable = smp.GetLookupTableForArray('ring', 1)
...
def openSensor():
    ...
    # RSEM4 Series
    #for typ in [37, 38, 39, 40]:
    #    if lidarType == typ :
    #        laserSelected = dialog.getLaserDefault480()
    #        app.curLidarType = lidarType
    # laserSelected = dialog.getLaserDefault480()
    if lidarType in [36, 37, 38 ,39]:
        app.curLidarType = lidarType
        app.targetFps = 20
    else:
        app.curLidarType = 0
    ...
    dialog = vvColumnSelectionDialog(getMainWindow())
    colSelected = dialog.getColumnSelection()
    ...
    sensor.GetClientSideObject().SetColumnSelection(colSelected)
    ...
    app.hardwareTimer.start(1)
    app.hardwareTimerForSensor.start(1)
    app.compRateTimer.start(1)
    app.speedTimer.start(1)
    app.pktLossTimer.start(1)
...
def openPCAP(filename, positionFilename=None):
    ...
    hardware_type = reader.GetClientSideObject().GetHardwareVerFromPcap()
    dialog = vvLaserSelectionDialog(getMainWindow())
    laserSelected = dialog.getLaserSelection()
    if hardware_type == "A0":
        laserSelected = dialog.getLaserDefault480()
    ...
    reader.GetClientSideObject().SetColumnSelection(colSelected)
...
def onSavePCAP():
    ...
    reader = getReader()
    if reader:
        reader.GetClientSideObject().saveFramesToPCAP(int(frameOptions.start), int(frameOptions.stop), fileName)
...
def onAbout():
    title = 'About RSView'
    text = '''<h1>RSView 4.3.13 for 0620</h1>
              Support Batch Laser/Col Selection.
              <br/>Copyright (c) 2017-2023, All Rights Reserved.'''

    QtGui.QMessageBox.about(getMainWindow(), title, text)

def onLicenseInfo():
    title = 'License Information'
    text = '''<h1>RSView 4.3.13</h1>
...
def close():
    ...
    app.hardwareLabel.setText('')
    app.compRateLabel.setText('')
    app.speedLabel.setText('')
    app.pktLossLabel.setText('')
    ...
    app.xSpinBox.setValue(0.0)
    app.ySpinBox.setValue(0.0)
    app.zSpinBox.setValue(0.0)
    app.pitchSpinBox.setValue(0.0)
    app.rollSpinBox.setValue(0.0)
    app.yawSpinBox.setValue(0.0)
...
def outputHvTimer():
    ...
    version_str = "unknown"
    if reader:
        version = reader.GetClientSideObject().OutputHardwareVersion()
        if version == 176:
            version_str = "B0"
        if version == 16:
            version_str = "A1"
        ...
        if version == 17:
            version_str = "A1.1"
        app.hardwareLabel.setText('  Hardware: %s  ' % version_str)
    elif sensor:
        version = sensor.GetClientSideObject().OutputHardwareVersion()
        if version == 176:
            version_str = "B0"
        if version == 16:
            version_str = "A1"
...
def outputCompRateTimer():
    reader = getReader()
    sensor = getSensor()

    if reader:
        #avg_rate = reader.GetClientSideObject().OutputCompressRateAvg()
        #min_rate = reader.GetClientSideObject().OutputCompressRateMin()
        #max_rate = reader.GetClientSideObject().OutputCompressRateMax()
        #lab_str = '  Comp-Rate Avg/Max/Min: {0}%,{1}%,{2}% '.format(round(avg_rate*100, 2), round(max_rate*100, 2), round(min_rate*100, 2))
        comp_info_str = reader.GetClientSideObject().OutputCompRateStr()
        lab_str = ' Comp-Rate Avg/Max/Min: Max/Min: {0}'.format(comp_info_str)
        app.compRateLabel.setText(lab_str)

    elif sensor:
        #avg_rate = sensor.GetClientSideObject().OutputCompressRateAvg()
        #min_rate = sensor.GetClientSideObject().OutputCompressRateMin()
        #max_rate = sensor.GetClientSideObject().OutputCompressRateMax()
        #lab_str = '  Comp-Rate Avg/Max/Min: {0}%,{1}%,{2}% '.format(round(avg_rate*100, 2), round(max_rate*100, 2), round(min_rate*100, 2))
        comp_info_str = sensor.GetClientSideObject().OutputCompRateStr()
        lab_str = ' Comp-Rate Avg/Max/Min: Max/Min: {0}'.format(comp_info_str)
        app.compRateLabel.setText(lab_str)
...
def outputSpeedTimer():
    reader = getReader()
    sensor = getSensor()

    if reader:
        speed = reader.GetClientSideObject().OutputNetSpeed()
        lab_str = ' speed: {0} Mbps '.format(round(speed, 2))
        app.speedLabel.setText(lab_str)

    elif sensor:
        speed = sensor.GetClientSideObject().OutputNetSpeed()
        lab_str = ' speed: {0} Mbps '.format(round(speed, 2))
        app.speedLabel.setText(lab_str)
...
def outputPktLossTimer():
    reader = getReader()
    sensor = getSensor()

    if reader:
        loss = reader.GetClientSideObject().OutputPktLoss()
        lab_str = ' loss:{0}% '.format(round(loss, 2))
        app.pktLossLabel.setText(lab_str)

    elif sensor:
        loss = sensor.GetClientSideObject().OutputPktLoss()
        lab_str = ' loss:{0}% '.format(round(loss, 2))
        app.pktLossLabel.setText(lab_str)
...
def playbackTick():
    ...
    if sensor is not None:
        ...
        if not app.colorByInitialized:
            sensor.UpdatePipeline()
            # custom 0620
            if colorByIntensityAndRings(sensor):
...
def createGrid(view=None):
    ...
    color_val = 0.5
    rep.DiffuseColor = [color_val, color_val, color_val]
    rep.Pickable = 0
    rep.Visibility = 0
...
def setupStatusBar():
    ...
    statusBar.addWidget(app.hardwareLabel)
    statusBar.addWidget(app.compRateLabel)
    statusBar.addWidget(app.speedLabel)
    statusBar.addWidget(app.pktLossLabel)
...
def onColumnSelection(show = True):
    ...
    if reader:
        reader.GetClientSideObject().SetColumnSelection(colSelected)
        app.pointsSizeSpinBox.setValue(app.pointSize+1)
        app.pointsSizeSpinBox.setValue(app.pointSize-1)
        smp.Render()
        smp.Render(getSpreadSheetViewProxy())
    elif sensor:
...
def onOpenSensorPosBox():
    #dialog = vvPosChangeDialog(getMainWindow())
    #if not dialog.exec_():
    #    return False
    #return True
    app.enableSensorPos = False
'''
    if (not app.sensor) and (not app.reader):
        app.enableSensorPos = False
        return
    if not app.enableSensorPos:
        app.posXLab.setVisible(True)
        app.posYLab.setVisible(True)
        app.posZLab.setVisible(True)
        app.posPitchLab.setVisible(True)
        app.posRollLab.setVisible(True)
        app.posYawLab.setVisible(True)
        app.actions['actionChangePosX'].setVisible(True)
        app.actions['actionChangePosY'].setVisible(True)
        app.actions['actionChangePosZ'].setVisible(True)
        app.actions['actionChangePosPitch'].setVisible(True)
        app.actions['actionChangePosRoll'].setVisible(True)
        app.actions['actionChangePosYaw'].setVisible(True)
        app.enableSensorPos = True
    else:
        app.posXLab.setVisible(False)
        app.posYLab.setVisible(False)
        app.posZLab.setVisible(False)
        app.posPitchLab.setVisible(False)
        app.posRollLab.setVisible(False)
        app.posYawLab.setVisible(False)
        app.actions['actionChangePosX'].setVisible(False)
        app.actions['actionChangePosY'].setVisible(False)
        app.actions['actionChangePosZ'].setVisible(False)
        app.actions['actionChangePosPitch'].setVisible(False)
        app.actions['actionChangePosRoll'].setVisible(False)
        app.actions['actionChangePosYaw'].setVisible(False)
        app.enableSensorPos = False
'''
...
def setupActions():
    ...
    app.actions['actionColumnSelection'].connect('triggered()', onColumnSelection)
    ...
    app.actions['actionOpenSensorPosSpins'].connect('triggered()', onOpenSensorPosBox)
    ...
    # color toolbar
    #
    viewSettingBar = mW.findChild('QToolBar', 'viewSettings')


    # pos X
    posChangeXLabel = QtGui.QAction('actionPosXLab', viewSettingBar)
    viewSettingBar.addAction(posChangeXLabel)
    app.actions['actionPosXLab'] = posChangeXLabel
    setActionIcon('actionPosXLab', IconPaths.pos_x_label)
    xSpinBox = QtGui.QDoubleSpinBox()
    xSpinBox.toolTip = "Modify Sensor X"
    xSpinBox.setMinimum(-100.0)
    xSpinBox.setMaximum(100.0)
    xSpinBox.setValue(0.0)
    xSpinBox.setSingleStep(1.0)
    xSpinBox.connect('valueChanged(double)', onPosXChanged)
    app.xSpinBox = xSpinBox
    app.actions['actionChangePosX'] = viewSettingBar.addWidget(xSpinBox)
    app.actions['actionChangePosX'].setVisible(False)
    app.posXLab = posChangeXLabel
    app.posXLab.setVisible(False)

    # pos Y
    posChangeYLabel = QtGui.QAction('actionPosYLab', viewSettingBar)
    viewSettingBar.addAction(posChangeYLabel)
    app.actions['actionPosYLab'] = posChangeYLabel
    setActionIcon('actionPosYLab', IconPaths.pos_y_label)
    ySpinBox = QtGui.QDoubleSpinBox()
    ySpinBox.toolTip = "Modify Sensor Y"
    ySpinBox.setMinimum(-100.0)
    ySpinBox.setMaximum(100.0)
    ySpinBox.setValue(0.0)
    ySpinBox.setSingleStep(1.0)
    ySpinBox.connect('valueChanged(double)', onPosYChanged)
    app.ySpinBox = ySpinBox
    app.actions['actionChangePosY'] = viewSettingBar.addWidget(ySpinBox)
    app.actions['actionChangePosY'].setVisible(False)
    app.posYLab = posChangeYLabel
    app.posYLab.setVisible(False)

    # pos Z
    posChangeZLabel = QtGui.QAction('actionPosZLab', viewSettingBar)
    viewSettingBar.addAction(posChangeZLabel)
    app.actions['actionPosZLab'] = posChangeZLabel
    setActionIcon('actionPosZLab', IconPaths.pos_z_label)
    zSpinBox = QtGui.QDoubleSpinBox()
    zSpinBox.toolTip = "Modify Sensor Z"
    zSpinBox.setMinimum(-100.0)
    zSpinBox.setMaximum(100.0)
    zSpinBox.setValue(0.0)
    zSpinBox.setSingleStep(1.0)
    zSpinBox.connect('valueChanged(double)', onPosZChanged)
    app.zSpinBox = zSpinBox
    app.actions['actionChangePosZ'] = viewSettingBar.addWidget(zSpinBox)
    app.actions['actionChangePosZ'].setVisible(False)
    app.posZLab = posChangeZLabel
    app.posZLab.setVisible(False)

    # pos Pitch
    posChangePitchLabel = QtGui.QAction('actionPosPitchLab', viewSettingBar)
    viewSettingBar.addAction(posChangePitchLabel)
    app.actions['actionPosPitchLab'] = posChangePitchLabel
    setActionIcon('actionPosPitchLab', IconPaths.pos_pitch_label)
    pitchSpinBox = QtGui.QDoubleSpinBox()
    pitchSpinBox.toolTip = "Modify Sensor Pitch"
    pitchSpinBox.setMinimum(-90.0)
    pitchSpinBox.setMaximum(90.0)
    pitchSpinBox.setValue(0.0)
    pitchSpinBox.setSingleStep(1.0)
    pitchSpinBox.connect('valueChanged(double)', onPosPitchChanged)
    app.pitchSpinBox = pitchSpinBox
    app.actions['actionChangePosPitch'] = viewSettingBar.addWidget(pitchSpinBox)
    app.actions['actionChangePosPitch'].setVisible(False)
    app.posPitchLab = posChangePitchLabel
    app.posPitchLab.setVisible(False)

    # pos Roll
    posChangeRollLabel = QtGui.QAction('actionPosRollLab', viewSettingBar)
    viewSettingBar.addAction(posChangeRollLabel)
    app.actions['actionPosRollLab'] = posChangeRollLabel
    setActionIcon('actionPosRollLab', IconPaths.pos_roll_label)
    rollSpinBox = QtGui.QDoubleSpinBox()
    rollSpinBox.toolTip = "Modify Sensor Roll"
    rollSpinBox.setMinimum(-180.0)
    rollSpinBox.setMaximum(180.0)
    rollSpinBox.setValue(0.0)
    rollSpinBox.setSingleStep(1.0)
    rollSpinBox.connect('valueChanged(double)', onPosRollChanged)
    app.rollSpinBox = rollSpinBox
    app.actions['actionChangePosRoll'] = viewSettingBar.addWidget(rollSpinBox)
    app.actions['actionChangePosRoll'].setVisible(False)
    app.posRollLab = posChangeRollLabel
    app.posRollLab.setVisible(False)

    # pos Yaw
    posChangeYawLabel = QtGui.QAction('actionPosYawLab', viewSettingBar)
    viewSettingBar.addAction(posChangeYawLabel)
    app.actions['actionPosYawLab'] = posChangeYawLabel
    setActionIcon('actionPosYawLab', IconPaths.pos_yaw_label)
    yawSpinBox = QtGui.QDoubleSpinBox()
    yawSpinBox.toolTip = "Modify Sensor Yaw"
    yawSpinBox.setMinimum(-360.0)
    yawSpinBox.setMaximum(360.0)
    yawSpinBox.setValue(0.0)
    yawSpinBox.setSingleStep(1.0)
    yawSpinBox.connect('valueChanged(double)', onPosYawChanged)
    app.yawSpinBox = yawSpinBox
    app.actions['actionChangePosYaw'] = viewSettingBar.addWidget(yawSpinBox)
    app.actions['actionChangePosYaw'].setVisible(False)
    app.posYawLab = posChangeYawLabel
    app.posYawLab.setVisible(False)

    #app.actions['actionOpenSensorPosSpins'].setVisible(False)

def refreshSensorPosition(position):
    lidarType = app.sensor.LidarType
    calibPath = app.sensor.CalibrationPath
    msopPort = app.sensor.MsopPort
    difopPort = app.sensor.DifopPort
    groupAddress = app.sensor.GroupAddress
    hostAddress = app.sensor.HostAddress
    userLayerBytes = app.sensor.UserLayerBytes
    tailLayerBytes = app.sensor.TailLayerBytes
...
def onPosXChanged(ax):
    if (not app.sensor) and (not app.reader):
        return
    #try:
    if app.sensor:
        #app.sensor.GetClientSideObject().ChangeSensorPositionX(ax)
        ay = app.ySpinBox.value
        az = app.zSpinBox.value
        a_yaw = app.yawSpinBox.value
        a_pitch = app.pitchSpinBox.value
        a_roll = app.rollSpinBox.value
        format_str = "{0} {1} {2} {3} {4} {5}".format(ax, ay, az, a_yaw, a_pitch, a_roll)
        refreshSensorPosition(format_str)
    if app.reader:
        app.reader.GetClientSideObject().ChangeSensorPositionX(ax)
        smp.Render()
        smp.Render(getSpreadSheetViewProxy())
...
def onPosYChanged(ay):
    if (not app.sensor) and (not app.reader):
        return
    try:
        if app.sensor:
            #app.sensor.GetClientSideObject().ChangeSensorPositionY(ay)
            ax = app.xSpinBox.value
            az = app.zSpinBox.value
            a_yaw = app.yawSpinBox.value
            a_pitch = app.pitchSpinBox.value
            a_roll = app.rollSpinBox.value
            ...
            refreshSensorPosition(format_str)
        if app.reader:
            app.reader.GetClientSideObject().ChangeSensorPositionY(ay)
        smp.Render()
        smp.Render(getSpreadSheetViewProxy())
    except Exception as e:
        pass

def onPosZChanged(az):
    if (not app.sensor) and (not app.reader):
        return
    try:
        if app.sensor:
            #app.sensor.GetClientSideObject().ChangeSensorPositionZ(az)
            ay = app.ySpinBox.value
            ax = app.xSpinBox.value
            a_yaw = app.yawSpinBox.value
            a_pitch = app.pitchSpinBox.value
            ...
            refreshSensorPosition(format_str)
        if app.reader:
            app.reader.GetClientSideObject().ChangeSensorPositionZ(az)
        smp.Render()
        smp.Render(getSpreadSheetViewProxy())
    except Exception as e:
        pass

def onPosPitchChanged(a_pitch):
    if (not app.sensor) and (not app.reader):
        return
    try:
        if app.sensor:
            #app.sensor.GetClientSideObject().ChangeSensorPositionPitch(a_pitch)
            ax = app.xSpinBox.value
            ay = app.ySpinBox.value
            az = app.zSpinBox.value
            a_yaw = app.yawSpinBox.value
            ...
            refreshSensorPosition(format_str)
        if app.reader:
            app.reader.GetClientSideObject().ChangeSensorPositionPitch(a_pitch)
        smp.Render()
        smp.Render(getSpreadSheetViewProxy())
    except Exception as e:
        pass

def onPosRollChanged(a_roll):
    if (not app.sensor) and (not app.reader):
        return
    try:
        if app.sensor:
            #app.sensor.GetClientSideObject().ChangeSensorPositionRoll(a_roll)
            ax = app.xSpinBox.value
            ay = app.ySpinBox.value
            az = app.zSpinBox.value
            a_yaw = app.yawSpinBox.value
            ...
            refreshSensorPosition(format_str)
        if app.reader:
            app.reader.GetClientSideObject().ChangeSensorPositionRoll(a_roll)
        smp.Render()
        smp.Render(getSpreadSheetViewProxy())
    except Exception as e:
        pass

def onPosYawChanged(a_yaw):
    if (not app.sensor) and (not app.reader):
        return
    try:
        if app.sensor:
            #app.sensor.GetClientSideObject().ChangeSensorPositionYaw(a_yaw)
            ax = app.xSpinBox.value
            ay = app.ySpinBox.value
            az = app.zSpinBox.value
            a_roll = app.rollSpinBox.value
            ...
            refreshSensorPosition(format_str)
        if app.reader:
            app.reader.GetClientSideObject().ChangeSensorPositionYaw(a_yaw)
        smp.Render()
        smp.Render(getSpreadSheetViewProxy())
    except Exception as e:
        pass

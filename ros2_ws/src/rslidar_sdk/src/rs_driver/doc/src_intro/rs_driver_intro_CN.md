...
+ E/EM平台雷达。如RSE1/RSEM4/RSEMX，是基于VCSEL与SPAD芯片的数字化架构雷达。
...
E/EM平台雷达的通道为垂直方向上扫描的点数，每个通道上的点连成一条线，通道数即线数。
...
+ E平台雷达的MSOP不携带角度信息，而是法向量与距离，可计算直角坐标系的坐标。
...
RSEM4是EM平台雷达。这里说明RSEM4的Decoder。

+ DecoderRSEM4的常量配置由成员函数getConstParam()生成。这个配置定义为静态本地变量。
+ 成员 `split_strategy_`是SplitStrategyBy类的实例，保存分帧策略。

##### 4.8.6.1 RSDecoderConstParam设置

|     常量参数     |                        值                        | 说明                              |
| :---------------: | :----------------------------------------------: | :-------------------------------- |
|     MSOP_LEN     |                       1084                       | MSOP Packet字节数                 |
|     DIFOP_LEN     |                       1310                       | DIFOP Packet字节数                |
|     MSOP_ID[]     |             {0x55, 0xAA, 0x5A, 0xA5}             | MSOP Packet标志字节，长度为4      |
|    MSOP_ID_LEN    |                        4                        | MSOP_LEN[]中标志字节的长度        |
|    DIFOP_ID[]    | {0xA5, 0xFF, 0x00, 0x5A, 0x11, 0x11, 0x55, 0x55} | DIFOP Packet标志字节，使用长度为3 |
|   DIFOP_ID_LEN   |                        3                        | DIFOP_LEN[]中标志字节的使用长度   |
|    BLOCK_ID[]    |                   {0x00, 0x00}                   | block标志字节，不使用             |
|     LASER_NUM     |                        1                        | 每个block的通道数，详见本表后注释 |
|  BLOCKS_PER_PKT  |                       260                       | 每Packet中260个Block              |
| CHANNEL_PER_BLOCK |                        1                        | RSEM4有260个通道                  |
|   DISTANCE_MIN   |                       0.5                       | 测距最小值，单位米                |
|   DISTANCE_MAX   |                      350.0                      | 测距最大值，单位米                |
|   DISTANCE_RES   |                      0.005                      | Packet中distance的解析度，单位米  |
|  TEMPERATURE_RES  |                        80                        | 雷达温度的初始值                  |

需要注意，对于EM4，每个MSOP内连续保存260个channel的数据，每两包520个channel构成1列，不开启ROI时1帧1200列，开启时1600列。为沿用之前结构，每包写作260个block，每个block 1个通道。事实上，改写为每包1个block，每个260个通道也是同样解析。

##### 4.8.6.2 DecoderRSEM4::decodeDifopPkt()

decodeDifopPkt() 解析DIFOP Packet。

+ 在DIFOP中，解析得到pitch角度pitch_angle以及yaw与pitch的偏置yaw_offset与surface_pitch_offset用于校准。

##### 4.8.6.3 DecoderRSEM4::decodeMsopPkt()

decodeMsopPkt()解析MSOP Packet。

+ 解析Packet中的 `temperature`字段，得到雷达温度，保存到 `temperature_`。
+ 根据用户设置，决定采用雷达时间戳还是主机本地时间戳。若采用雷达时间戳，调用parseTimeUTCWithUs()得到Packet的时间戳，保存到本地变量 `pkt_ts`。
+ 调用SplitStrategyBySeq::newPacket()，检查是否分帧。如果是，调用回调函数 `cb_split_frame_`，通知使用者。
  `cb_split_frame_`应该转移点云 `pont_cloud_`，并重置它。
+ 遍历Packet内所有的Block。

  + 从Block相对于Packet的偏移，得到Block的时间戳。Block内的所有Channel的时间戳都是这个时间戳，当然 RSEM4每Block仅1个channel，为通用写法。
  + 单回波每2包、双回波每4包构成1列520通道，计算得到通道号。根据VCSEL序号与Channel计算得到yaw和pitch。
+ 遍历Block内所有的Channel。
  ...
  + 计算点云坐标(`x`, `y`, `z`)。
  + 调用transfromPoint()做坐标转换。
  + 设置点云的 `intensity`、`timestamp`，`ring`，`feature`。
  + 将点保存到点云 `point_cloud_`的尾部。
...
##### 4.8.7.1 DecoderFactory::creatDecoder()
